...
output = 'snapshot comparision.txt'
...
try:
    ...
    print (snapshot1_file, 'used')
except:
    print (snapshot1_file, 'does not exist, exiting')
    quit()
...
try:
    ...
    print (snapshot2_file, 'used')

except:
    print (snapshot2_file, 'does not exist, exiting')
    quit()
...
for job_no_val in job_no_list:
    ...
    print ('=' * 80)
    print (job_no_val)
    print (snapshot1,' External Fee:',ext_fee_val,'Intercompany Fee:',interco_fee_val,'Subcontract Fee', subcon_fee_val)
    print (snapshot2,' External Fee:',ext_fee2_val,'Intercompany Fee:',interco_fee2_val,'Subcontract Fee', subcon_fee2_val)
    print ('=' * 80)
    ...
    output_file.write(newline)
    output_file.write(newline)
    output_file.write(snapshot1 + ' - External Fee: ' + ext_fee_nice)
    ...
    output_file.write(newline)                     
    output_file.write(snapshot2 + ' - External Fee: ' + ext_fee2_nice)
...
new_inv_job_list_string = ('The following ' + str(new_inv_job_count) + ' jobs are in the ' + snapshot2 + ' file but not in ' + snapshot1 + '\n\n')
...
print('Delta ',snapshot1,' vs ',snapshot2,' totals')
print(delta_sum_str)
print(delta_total_str)
print(delta_charge_str)
print(seperate)
output_file.write('Delta ' + snapshot1 + ' vs ' + snapshot2 + ' totals\n')
...
if matched_job + non_matched_job == unique_jobno_count:
    print('Success, all of the jobs in ', snapshot1, ' were found in ', snapshot2)
    output_file.write('Success, all of the jobs in ' + snapshot1 + ' were found in ' + snapshot2)
    output_file.write(newline * 2)
elif matched_job + non_matched_job == unique_jobno_count:
    print('CAUTION, there are missing jobs on the fee tracker')
    output_file.write('CAUTION, there are missing jobs on the in ' + snapshot2 + '\n')
    output_file.write(newline * 2)
